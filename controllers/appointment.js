...
exports.createAppointment = async (req, res) => {
  ...
  try {
    ...
    const isSlotCount = await Appointment.countDocuments({ bookingSlot });

    const daySlot = await DaySlot.findById(bookingSlot);
    const maxBookingsPerSlot = daySlot ? daySlot.maxBookingsPerSlot : 0;

    const isSlot = maxBookingsPerSlot - isSlotCount;
